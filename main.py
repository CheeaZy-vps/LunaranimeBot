...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    user_id = query.from_user.id
                                                               if user_id not in ADMIN_USER_ID:                               await query.answer("❌ Admin only!", show_alert=True)
        return

    await query.answer("📤 Mengirim source code...")       
    files = [
        ('LunaranimeBot-AIVersion.py', '🤖 Main Bot'),
        ('ApiLunaranime.py', '🌙 API Wrapper'),                    ('.LunaranimeBot.db.json', '📋 Dependencies'),             ('privacy-policy.html', '📋 Dependencies')
    ]                                                      
    for file_path, caption in files:
        if Path(file_path).exists():
            try:
                message = await context.bot.send_document(                     chat_id=query.message.chat_id,
                    document=open(file_path, 'rb'),                            caption=f"{caption}\nPath: `{file_path}`",
    ...
    await MessageManager.send_temp(
        context,
        update.effective_chat.id,                                  "✅ Semua file source code telah dikirim! 🎉",
        delay=60
    )

ADMIN_UPLOAD_DIR = Path("admin_uploads")                   ADMIN_UPLOAD_DIR.mkdir(exist_ok=True)                                                                                 async def admin_file_receiver_hendler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin only file receiver"""                             user_id = update.effective_user.id
    if user_id not in ADMIN_USER_ID:
        await update.message.reply_text("❌ Admin only!")
        return                                                                                                            message = update.message                                   file = message.document                                    if not file: return
                                                               # Validasi
    allowed_types = ['.pdf', '.jpg', '.png', '.zip', '.py', '.txt', '.json', '.mp4', '.mp3']                              if not any(file.file_name.lower().endswith(ext) for ext in allowed_types):
        await message.reply_text("❌ Tipe file tidak diizinkan!")
        return
                                                               if file.file_size > 20 * 1024 * 1024:  # 20MB
        await message.reply_text("❌ File terlalu besar! Max 20MB")                                                           return

    # Sanitize filename                                        safe_name = re.sub(r'[^\w\-\.]', '_', file.file_name)
    file_path = ADMIN_UPLOAD_DIR / safe_name

    try:                                                           telegram_file = await context.bot.get_file(file.file_id)                                                              await telegram_file.download_to_drive(file_path)   
        # Hapus pesan user
    ...
    except Exception as e:                                         await MessageManager.send_temp(
            context,
            update.effective_chat.id,
...
def atexit() -> None:                                          import atexit
    UIColor.clear_screen()                                     UIColor.set_title('LunaranimeBot')
    ...
    app.add_handler(MessageHandler(                                filters.PHOTO & filters.User(user_id=ADMIN_USER_ID),
